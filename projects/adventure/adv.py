...
class AdventureTraversal:
    '''
    Takes the roomGraph provided above and generates a traversal path using
    cardinal directions to visit all rooms in the graph.
    Requires `from collections import deque` along with graph generating code.
    '''

    # self.adv = player
    # player.currentRoom.id - Numeric representation of the current room
    ...
    def explore_room(self, previous_room=None, entered_dir=None):
        ...
        # TODO: Track down the "You cannot move in that direction." bug. Probably in self.traverse_room_path().
    ...
    def traverse_room_path(self, room_path):
        '''
        Takes in a list of room numbers, and walks them from the current room
        (which should be the first element in the list) to the last entry.
        '''

        # Sync check to make sure the first element of the path is in fact our
        # current room, otherwise bail out.
        if room_path[0] == self.adv.currentRoom.id:
            # We'll need to keep track of the previous room so we know from where
            # to navigate
            previous_room = None
            # Start going through the room path
            for cur_path_room in room_path:
                if self.debug == True:
                    print(f'Previous path room: {previous_room}')
                    print(f'Current path room: {cur_path_room}')
                # We don't need to go from the first room to the first room, so
                # skip it, making note of it for the second iteration.
                if cur_path_room == room_path[0]:
                    previous_room = cur_path_room
                    continue
                else:
                    # Get the direction towards the next (current) room we want to go to
                    dir_to_go = None
                    for exit_dir in self.dungeon_map[previous_room].keys():
                        if self.dungeon_map[previous_room][exit_dir] == cur_path_room:
                            dir_to_go = exit_dir
                            break
                    # Travel in that direction
                    self.adv.travel(dir_to_go)
                    # Explore the room
                    self.explore_room(previous_room, dir_to_go)
        else:
            print(
                f'ERROR! Rooms out of sync! Path room {room_path[0]} != current room {self.adv.currentRoom.id}!'
            )

    def path_forward(self, dir_to_move):
        '''
        Utilizes a modified DFT to navigate to rooms with unexplored exits
        until one is entered with nothing but explored rooms.
        '''
    ...
    def path_back(self):
        '''
        Utilizes a BFS to find a room with an unexplored exit.
        '''

        '''
        __Notes__
        BFS from the current room until we get back a path to a room with an
        unexplored exit.
        '''

        # Make sure we don't wander the dungeon for all eternity looking for a
        # non-existant exit.
        if len(self.dungeon_map) < len(self.dungeon):
            # Set up for traversal and tracking visited rooms while looking for
            # the elusive '?'. Living that FIFO life.
            q = deque()
            q.append([self.adv.currentRoom.id])
            visited = set()
            # while there's still things left in the queue, do a BFS
            while len(q) > 0:
                # TODO: The contents of this can overall be optimized to return when the wanted room is found immediately instead of when the path with it gets popped from the queue, saving some runtime depending how deep in the queue the wanted route might be.
                # Grab a path list and set "current room" to the last element in it
                room_path = q.popleft()
                current_room = room_path[-1]
                # If we haven't been to this room yet in our backtracking,
                # start building possible paths from its exits.
                if current_room not in visited:
                    # If there's an unexplored exit in current_room, we have a
                    # good path to traverse back.
                    if '?' in self.dungeon_map[current_room].values():
                        self.traverse_room_path(room_path)
                        break
                    # We've now seen the current room, so make sure we don't
                    # come back
                    visited.add(current_room)
                    # Spawn some new potential room paths with adjacent rooms
                    for adjacent_room in self.dungeon_map[current_room].values():
                        # if adjacent_room not in visited:
                        possible_path = list(room_path)
                        possible_path.append(adjacent_room)
                        q.append(possible_path)

    def partial_traversal(self):
    ...
    def traverse(self):
        '''
        Generates a graph and a path to traverse it. Returns the traversal path.
        '''
