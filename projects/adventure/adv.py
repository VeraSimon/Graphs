...
class AdventureTraversal:
    ...
    def __init__(self, adventurer, traversal_path, room_graph):
        self.rogue = adventurer
        self.traversal_path = traversal_path
        self.explored_graph = {}
        self.room_graph = room_graph
        self.previous_room = None

    def explore_room(self, dirMoved):
        inverseDirections = {'n': 's', 's': 'n', 'e': 'w', 'w': 'e'}
        self.previous_room = self.rogue.currentRoom
        if dirMoved in self.rogue.currentRoom.getExits():
            self.rogue.travel(dirMoved)
            self.traversal_path.append(dirMoved)
            ...
            if self.explored_graph[self.previous_room.id][dirMoved] == '?':
                self.explored_graph[self.previous_room.id][dirMoved] = self.rogue.currentRoom.id
                self.explored_graph[self.rogue.currentRoom.id][inverseDirections[dirMoved]] = \
                    self.previous_room.id
    ...
    def path_forward(self, dir_to_move):
        ...
        while len(s) > 0:
            d = s.pop()
            self.explore_room(d)
            if self.rogue.currentRoom.id not in visited:
                visited.add(self.rogue.currentRoom.id)
                for ex in self.rogue.currentRoom.getExits():
    ...
    def traverse(self):
        ...
        while len(self.explored_graph) < len(self.room_graph):
            ...
            for ex in self.explored_graph[self.rogue.currentRoom.id].values():
                ...
                # if the room doesn't exist, initialize in user room graph with '?' exits
                # if player.currentRoom.id not in self.explored_graph:
                #     self.explored_graph[player.currentRoom.id] = {
                #         ex: '?' for ex in self.rogue.currentRoom.getExits()}
                    # for exit in player.currentRoom.getExits():
                    # 	graph[player.currentRoom.id][exit] = '?'
                    # if self.previous_room is not None:
                    #     self.explored_graph[self.previous_room][dirToMove] = player.currentRoom.id
                    #     oppositeDirection = self.inverseDirections[dirToMove]
                    #     self.explored_graph[player.currentRoom.id][oppositeDirection] = self.previous_room
